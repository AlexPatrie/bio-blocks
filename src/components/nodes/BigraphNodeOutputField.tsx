...
interface BigraphNodeOutputFieldProps {
  data: BigraphNode;
  portName: string;
  // handleInputChange: (
...
export function BigraphOutputNodeField({ data, portName }: BigraphNodeOutputFieldProps) {
  const [editMode, setEditMode] = useState(false);
  const [tempValue, setTempValue] = useState(data.outputs[portName]); // local state for editing
  
  // get original value
  const originalValue = data.outputs[portName];
  ...
  const handleKeyDown = useCallback((keyEvent: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (keyEvent.key === "Enter") {
      ...
      if (originalValue !== tempValue) {
        data.outputs[portName] = tempValue;
      }
    }
  }, []);
